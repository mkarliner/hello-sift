...
var MyController = (function (SiftController) {
  ...
  MyController.prototype.loadView = function loadView (state) {
    ...
    switch (state.type) {
      ...
      case 'summary':
			return {
				html: 'summary.html',
				data: this.getWebhook().then(function (x) { return ({ name: 'no-one', hook_uri: x}); })};
      default:
  ...
  MyController.prototype.onStorageUpdate = function onStorageUpdate (value) {
    ...
    return this.getName().then(function (xe) {
	  ...
	  console.log("OSU: ", xe)
      this$1.publish('name', xe);
    });
  };
  
  MyController.prototype.getWebhook = function getWebhook () {
   ...
   MyController.prototype.getName = function getName () {
